"""
Thin integrated vision-navigation controller for the G1 web / voice demo.

This controller does NOT own the per-cycle navigation loop.  That loop lives
inside the task objects (``tasks.VLNTask.run`` and friends, which call
``self.robot.execute_trajectory``).  The controller's only jobs are:

- build / hold the active task,
- launch ``task.run()`` on a daemon thread for the web demo,
- mirror the source threading + SocketIO emit pattern
...
class IntegratedVisionNavController:
    ...
    def start_new_task(self, instruction: str) -> Tuple[bool, str]:
        ...
        SocketIO when present.
        """
        # Stop existing task if running.
        if self.current_task is not None and getattr(
            self.current_task, "is_task_running", False
        ):
