"""
iPlanner Agent - Neural Network Trajectory Planner
====================================================
Loads the iPlanner model and performs depth-based trajectory planning.
"""
...
class IPlannerAgent:
    def __init__(
        ...
        self.transform = transforms.Resize(self.img_input_size, antialias=None)

    def load_model(self, model_path: str, model_config_path: str):
        """Load iPlanner model weights and configuration."""
    ...
    def process_depth(self, depth: torch.Tensor) -> torch.Tensor:
        """Resize and sanitize depth tensor for model input."""
    ...
    def process_image(self, image: torch.Tensor) -> np.ndarray:
        """Convert image tensor to NumPy HWC format."""
        image_np = image.permute(0, 2, 3, 1).cpu().numpy()
        return image_np

    def plan(self, image, goal_robot_frame):
        """Run iPlanner forward pass and generate trajectory."""
        with torch.no_grad():
            keypoints, fear = self.net(
                self.process_depth(image), goal_robot_frame
            )
    ...
    def step_pointgoal(
        ...
        """Single planning step given a depth image and goal in robot frame."""
        with torch.no_grad():
            ...
            keypoints, traj, fear = self.plan(
                tensor_dep_image, tensor_goal_robot_frame
            )
            return keypoints, traj, fear
