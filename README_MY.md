...
| `vlnce_baselines/utils/api_openai.py` | 修改 | `label` 参数统一统计分组；日志函数迁移到 logging.py |
...
| `env.sh` | 修改 | 日志控制（已升级为 preset 系统，见上文 LAVIRA_LOG） |
| `vlnce_baselines/utils/api.py` | 修改 | 重新导出从 logging.py |
| `vlnce_baselines/env/env_utils.py` | 修改 | 进度条条件替换 |
...
| 文件 | 状态 | 说明 |
|------|------|------|
| `vlnce_baselines/agent_v3.py` | 新增 | `VLMReasoningAgentV3`，单次调用 judge() + select_one() |
| `vlnce_baselines/prompts/prompts_objnav_v3.py` | 新增 | V3 英文 prompt 模板 (PROMPT_JUDGE + PROMPT_SELECT_ONE) |
| `vlnce_baselines/ZS_Evaluator_mp.py` | 修改 | +rollout_v3(), 4 方向采集、came_from_direction、bbox history |
| `run_mp.py` | 修改 | `--rollout-v3` CLI 参数 |
| `vlnce_baselines/utils/visualization.py` | 修改 | `_save_rgb_with_bbox` 增加 `label` 参数 |
| `vlnce_baselines/utils/logging.py` | 新增 | 统一日志系统（V1/V2/V3 共用） |
| `env.sh` | 修改 | 新 preset 系统替换旧 env var |
...
V3 使用统一的日志系统：`log_plan()` 输出 `-PLAN`，`log_fmm()` 输出 `---FMM`，`log_act()` 输出 `----ACT`。开关通过 `LAVIRA_LOG_PLAN/FMM/ACT` 控制，兼容 V1/V2。