...
class VLNCEDatasetV1(Dataset):
    ...
    def check_config_paths_exist(config) -> bool:
        logger.info(config.DATA_PATH.format(split=config.SPLIT))
        logger.info(os.path.exists(config.SCENES_DIR))
...
class RxRVLNCEDatasetV1(Dataset):
    ...
    def __init__(self, config = None) -> None:
        ...
        if config.EPISODES_ALLOWED is not None:
            ...
            # logger.info([episode.episode_id for episode in self.episodes])
...
class ObjectnavDatasetV1(ObjectNavDatasetV1):
    r"""Class inherited from ObjectNavDatasetV1 that loads a Object
    Navigation dataset.
    """
    def from_json(self, json_str: str, scenes_dir: Optional[str] = None) -> None:
        deserialized = json.loads(json_str)
        # Extra fields present in newer episode JSON (e.g. OVON) that older
        # habitat-lab Episode classes don't accept. Strip them before forwarding.
        extra_episode_fields = {
            "scene_dataset_config",
            "additional_obj_config_paths",
            "children_object_categories",
        }
        if "episodes" in deserialized:
            for episode in deserialized["episodes"]:
                for k in extra_episode_fields:
                    episode.pop(k, None)

        cleaned_json_str = json.dumps(deserialized)
        super().from_json(cleaned_json_str, scenes_dir)
    ...
    def __init__(self, config) -> None:
        ...
        if config.EPISODES_ALLOWED is not None:
            ep_ids_before = {int(ep.episode_id) for ep in self.episodes}
            ep_ids_to_purge = ep_ids_before - set([int(id) for id in config.EPISODES_ALLOWED])
            self.episodes = [
                episode
                for episode in self.episodes
                if int(episode.episode_id) not in ep_ids_to_purge
            ]
            # logger.info(f"EPISODES_ALLOWED: {config.EPISODES_ALLOWED}")
            # logger.info(f"ep_ids_before: {ep_ids_before}")
            # logger.info(f"ep_ids_to_purge: {ep_ids_to_purge}")
            # logger.info(f"final episodes: {[episode.episode_id for episode in self.episodes]}")