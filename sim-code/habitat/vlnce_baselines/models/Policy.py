...
class FusionMapPolicy(nn.Module):
    ...
    def _get_action(self, 
        ...
        # logger.info('[DBG]Agent position:', position)
        # logger.info('[DBG]waypoint:', waypoint)
        # logger.info('[DBG]sub waypoint:',sub_waypoint)
        # logger.info('[DBG]waypoint_vector:', waypoint_vector)
        ...
        if stop and self.fixed_destination is not None:
            ...
            # logger.info('[DBG]relative_angle:', relative_angle)
        ...
        if self.visualize:
            ...
            #cv2.waitKey(1)
        if True:
            # Handle episode ID which might be a string like '00813-svBbv1Pavdk'
            # Use string formatting %s instead of %d
    ...
    def _search_destination(self, 
        ...
        for i, destination in enumerate(destinations):
            ...
            if destination_waypoint is not None:
                ...
                logger.info(f"value part: {value_part}")
                logger.info(f"confidence part: {confidence_part}")
                logger.info(f"destination value: {destination_value}")
                logger.info(f"destination waypoint: {destination_waypoint}")
    ...
    def forward(self, 
        ...
        logger.info(f"current_position's value: {value_map[min(int(y), self.map_shape - 1), min(int(x), self.map_shape - 1)]}")
        logger.info(f"current pose: {full_pose}")
        ...
        if search_destination:
            ...
            if destination_waypoint is not None and score >= self.max_destination_socre:
                logger.info(f"!!!!!!!find destination: {destination_waypoint}")
