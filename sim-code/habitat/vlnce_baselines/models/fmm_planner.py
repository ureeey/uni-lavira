...
class FMMPlanner:
    ...
    def get_short_term_goal(self, agent_position: np.ndarray, fixed_destination: np.ndarray, pad: int=5, ) -> Tuple:
        ...
        if subset.shape != mask.shape:
            logger.info("subset and mask have different shape")
            logger.info(f"subset shape:{subset.shape}, mask shape:{mask.shape}")
            logger.info(f"current positon:{agent_position}")
            return x, y, True
        subset *= mask
        subset += (1 - mask) * 1e5
