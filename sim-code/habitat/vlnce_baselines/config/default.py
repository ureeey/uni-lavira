...
_C.SAVE_RGB_IMAGES = False
_C.RGB_SAVE_DIR = "saved_rgb_images"
_C.DEBUG_LOGGING = False
_C.DEBUG_LOG_DIR = "logs/debug_logs"
_C.TASK_TYPE = "VLN"  # one of {"VLN", "OBJNAV", "EQA"}
...
# The TODO List Memory (TDM) toggle. Default True = the full TT method.
_C.EVAL.USE_TODO_LIST = True
# The Second Chance Backtrack (SCB) toggle. Default True = the full TT method.
_C.EVAL.BACKTRACK_SECOND_CHANCE = True
# Episodes are dispatched dynamically via a shared mp.Queue so fast workers can
# grab the next pending episode instead of being stuck with a fixed slice. Each
# worker rebuilds its habitat env per episode (~5-15s overhead) but reuses its
# trainer/model state across episodes.
_C.EVAL.DYNAMIC_QUEUE = True
# The LA navigation history is a dense "every N steps a forward-view RGB" stream.
_C.EVAL.USE_CONTINUOUS_HISTORY = True
# Step interval for sampling continuous-history RGB frames. Smaller value = denser
# history (more LA input tokens, finer temporal context).
_C.EVAL.HISTORY_INTERVAL = 2
...
def get_config(
    ...
    assert config.TASK_TYPE in {"VLN", "OBJNAV", "EQA"}, (
        f"Unknown TASK_TYPE: {config.TASK_TYPE!r}. Expected one of VLN, OBJNAV, EQA."
    )
