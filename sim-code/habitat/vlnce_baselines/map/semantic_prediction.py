    ...
    def segment(self, image: VisualObservation, **kwargs) -> Tuple[np.ndarray, List[str], np.ndarray]:
        ...
        for i, (_, _, confidence, class_id, _) in enumerate(detections):
            if class_id is not None:
                ...
                # Ensure class_id is not None for supervision annotator
                # Use a large integer ID for "unknown" to avoid conflict with valid classes (assuming < 1000 classes)
                # Supervision requires non-negative integers
                detections.class_id[i] = 9999
        ...
        # t4 = time.time()
        # logger.info("grounding dino: ", t2 - t1)
        # logger.info("process detections: ", t3 - t2)
        # logger.info("sam: ", t4 - t3)
