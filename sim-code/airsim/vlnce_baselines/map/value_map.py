...
class ValueMap(nn.Module):
    ...
    def _create_model(self):
        ...
        """
        assert False, "This method is deprecated. Use _load_model_from_disk instead."
        # self.model = torch.load()
        
    def _load_model_from_disk(self):
        # self.model = torch.load(self.config.BLIP2_MODEL_DIR, map_location="cpu", weights_only=False).to(self.device)
        # self.vis_processors = torch.load(self.config.BLIP2_VIS_PROCESSORS_DIR, weights_only=False)["eval"]
        # self.text_processors = torch.load(self.config.BLIP2_TEXT_PROCESSORS_DIR, weights_only=False)["eval"]
    ...
    def get_blip_value(self, image: Image, caption: str) -> torch.Tensor:
        ...
        print(itc_out.logits_per_image)

        return itc_out.logits_per_image[0][0]
