...
class Semantic_Mapping(nn.Module):
    ...
    def _visualize(self, 
        ...
        if self.last_loc is not None and len(self.last_loc) > id and self.last_loc[id] is not None:
            ...
            # If no previous location, just mark current position as visited
            print("No previous location available, skipping trajectory line drawing")
        ...
        1: obstacles
        2: agent trajectory
        3: visited areas (agent path)
        4: target/goal point
        5 ~ num_detected_class+4: detected objects
        """
        semantic_map += 5
        ...
        semantic_map[vis_mask] = 3  # agent trajectory (currently using goal color)
        
        # Draw target/goal point if provided - use an 'X' symbol instead of color
        goal_position = None  # Store goal position for later drawing
        # print('Goal position: ', goal)
        if goal is not None:
            try:
                # Convert goal coordinates from full map to local map coordinates
                if hasattr(goal, 'cpu'):
                ...
                if len(goal_coords) >= 2:
                    ...
                    # First check if goal is within full map bounds
                    full_map_height, full_map_width = self.map_shape  # (480, 480)
                    if (0 <= goal_map_x < full_map_height and 
                        0 <= goal_map_y < full_map_width):
                        
                        # Convert from full map coordinates to local map coordinates
                        local_goal_x = goal_map_x - gx1
                        local_goal_y = goal_map_y - gy1
                        print('Local goal coordinates:', local_goal_x, local_goal_y)
                        
                        # Check if goal is within local map bounds
                        if (0 <= local_goal_x < semantic_map.shape[0] and 
                            0 <= local_goal_y < semantic_map.shape[1]):
                            

                            goal_position = (local_goal_y, local_goal_x)  # Note: (x, y) format for OpenCV
            except Exception as e:
                print(f"Error processing goal coordinates: {e}")
        ...
        # Convert to numpy array for OpenCV drawing operations
        sem_map_vis = np.array(sem_map_vis)


        # Draw 'X' symbol at goal position if available
        if goal_position is not None:
            x, y = goal_position
            # Draw an 'X' using two lines
            line_thickness = 2
            line_color = (0, 0, 0)  # Black color (RGB format)
            cross_size = 8  # Size of the X symbol
            
            # Draw first diagonal line (top-left to bottom-right)
            cv2.line(sem_map_vis, 
        ...
        # print(sem_map_vis.shape)
        # sem_map_vis is already in RGB format from numpy array conversion
    ...
    def create_vlm_map_from_state(
        ...
        """
        Create a coordinate-accurate map image from the current internal state (self.local_map, self.state, etc.),
        designed for VLM use with high contrast and simplified information.
        This function signature is compatible with _visualize.

        Args:
            id (int): environment batch index to visualize.
            goal (Tensor, optional): global map coordinates of the goal.
            output_size (tuple, optional): output image size.
            visited_targets (list, optional): list of historical target positions.
        ...
        if target_legend_info:
            ...
            rows_needed = (total_items + max_items_per_row - 1) // max_items_per_row
            ...
            extended_map = np.full((extended_height, output_size[0], 3), (250, 250, 250), dtype=np.uint8)
            ...
            item_width = (output_size[0] - 30) // max_items_per_row
            ...
            for i, item in enumerate(target_legend_info):
                ...
                cv2.circle(extended_map, (item_x, item_y - 5), 6, (255, 255, 255), 2)
