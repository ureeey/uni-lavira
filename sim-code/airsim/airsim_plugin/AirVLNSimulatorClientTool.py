...
class AirVLNSimulatorClientTool:
    ...
    def run_call(self, airsim_timeout: int=300) -> None:
        ...
        def _run_command(index, socket_client: msgpackrpc.Client):
            logger.info(f'Starting scene launch, machine{index}: {socket_client.address._host}:{socket_client.address._port}')
            ...
            if result[0] == False:
                logger.error(f'Failed to launch scene, machine: {socket_client.address._host}:{socket_client.address._port}')
                raise Exception('Failed to launch scene')
            assert len(result[1]) == 2, 'Failed to launch scene'
            ...
            assert str(ip) == str(socket_client.address._host), 'Failed to launch scene'
            assert len(ports) == len(self.machines_info[index]['open_scenes']), 'Failed to launch scene'
            ...
            logger.info(f'Scene launch completed, machine{index}: {socket_client.address._host}:{socket_client.address._port}')
            return ports
        ...
        after = time.time()
        diff = after - before
        logger.info(f"Startup time: {diff}")
    ...
    def closeScenes(self):
        try:
            ...
            def _run_command(index, socket_client: msgpackrpc.Client):
                logger.info(f'Starting to close all scenes, machine{index}: {socket_client.address._host}:{socket_client.address._port}')
                result = socket_client.call('close_scenes', socket_client.address._host)
                logger.info(f'Finished closing all scenes, machine{index}: {socket_client.address._host}:{socket_client.address._port}')
                return
    ...
    def move_path_by_waypoints(self, waypoints_list, start_states):
        ...
        def move_path(airsim_client: airsim.VehicleClient, waypoints, start_state):
            ...
            print(f"[DEBUG] Setting kinematics orientation: {[start_state.orientation.w_val, start_state.orientation.x_val, start_state.orientation.y_val, start_state.orientation.z_val]}")
            ...
            target_idx = 5
            current_idx = 0
            pos_queue = deque(maxlen=40)  
            start_time = time.perf_counter()
            collision = False
            distance = 10000
            while True:
                time.sleep(0.005)
                if time.perf_counter() - start_time > 200:  # timeout guard
                    print('=' * 80)
                    print('⚠️  termination reason: path execution timeout(exceeded 200 seconds)')
                    print(f'   - completed waypoints: {current_idx}/{target_idx}')
                ...
                # print(f"[DEBUG] AirSim orientation: {state_info['orientation']}")

                
                if state_info['collision']['has_collided']:
                    print('=' * 80)
                    print('⚠️  termination reason: drone collision detected')
                    print(f'   - collided object: {state_info["collision"]["object_name"]}')
                    print(f'   - collision position: {state_info["position"]}')
                    print(f'   - completed waypoints: {current_idx}/{target_idx}')
                    print(f'   - target waypoint: [{target.x_val}, {target.y_val}, {target.z_val}]')
                    print('=' * 80)
                    collision = True
                    break
                ...
                if len(pos_queue) == pos_queue.maxlen:
                    ...
                    if delta_distance < 0.1:
                        print('=' * 80)
                        print('⚠️  termination reason: drone did not move within 20 frames during action execution')
                        print(f'   - detection window: 40 frames (about 0.2s)')
                        print(f'   - actual displacement: {delta_distance:.4f} m')
                        print(f'   - threshold: 0.1 m')
                        print(f'   - current position: {position}')
                        print(f'   - target waypoint: [{target.x_val}, {target.y_val}, {target.z_val}]')
                        print('=' * 80)
                        collision = True
                ...
                if new_distance > distance:
                    ...
                    if current_idx == target_idx:
                        print('✅ Path execution succeeded: all waypoints completed')
                        print(f'   - completed waypoint count: {target_idx}')
        ...
        def _getImages(airsim_client: airsim.VehicleClient):
            ...
            while True:
                try:
                    ...
                    time_sleep_cnt += 1
                    logger.error("image retrieval error: " + str(e))
