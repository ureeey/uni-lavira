    ...
    def check_collision_by_depth(self, episodes, current_observations, collisions, dones):
        for i, prev_episode in enumerate(episodes):
            ...
            if collisions[i]:
                ...
                print('=' * 80)
                print(f'⚠️  Collision status already marked (Episode {i})')
                print(f'   - collision type: already (collision previously detected)')
                print(f'   - status: Done = {dones[i]}')
                print('=' * 80)
                if not dones[i]:
                    dones[i] = True
                continue
            ...
            if collision_type is not None:
                print('=' * 80)
                print(f'⚠️  Environment collision detected (Episode {i})')
                print(f'   - collision type: {collision_type}')
                if collision_type == 'tiny diff':
                    print(f'   - reason: depth image change is too small(all cameras < 3)')
                    print(f'   - depth changes by camera: {[f"{d:.2f}" for d in diffs]}')
                elif collision_type == 'close':
                    print(f'   - reason: very close obstacle detected(pixels with depth <= 0.39m exceed 20%, excluding the downward camera)')
                    print(f'   - triggered cameras: {close_collision_cameras}')
                    print(f'   - depth changes by camera: {[f"{d:.2f}" for d in diffs]}')
                elif collision_type == 'distance':
                    print(f'   - reason: drone displacement is too small(< 0.1m)')
                    print(f'   - actual displacement: {distance:.4f} m')
                print(f'   - current position: {current_episode[-1]["sensors"]["state"]["position"]}')
                print(f'   - previous position: {prev_episode[-1]["sensors"]["state"]["position"]}')
                print('=' * 80)
            collisions[i] = np.all(diffs < 3) or close_collision or distance < 0.1
