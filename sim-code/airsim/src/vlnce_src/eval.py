...
def eval(model_wrapper: BaseModelWrapper, assist: Assist, eval_env: AirVLNENV, eval_save_dir):
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            for batch_idx in range(eval_env.batch_size):
                if len(batch_state.episodes[batch_idx]) > 0:
                    instruction = batch_state.episodes[batch_idx][0].get('instruction', 'N/A')
                    logger.info(f'  Episode {batch_idx}: {instruction}')
            logger.info('='*80)
            ...
            for t in range(int(args.maxWaypoints) + 1):
                ...
                #         observations_list, done, collision, oracle_success = output
                #         instruction = observations_list[-1].get('instruction', 'N/A')
                #         logger.info(f'Batch {batch_idx} Instruction: {instruction}')
