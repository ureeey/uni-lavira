...
class AirVLNENV:
    ...
    def _changeEnv(self, need_change: bool = True):
        ...
        assert len(using_map_list) == self.batch_size, 'error'
        ...
        while True:
            try:
                ...
                logger.error("scene startup failed {}".format(e))
                time.sleep(3)
            except:
                logger.error('scene startup failed')
                time.sleep(3)
    ...
    def makeActions(self, waypoints_list):
        ...
        for batch_idx, batch_result in enumerate(batch_results):
            logger.info(f"[makeActions] Episode {batch_idx}: AirSim returned {len(batch_result)}  status entries")
            if 0 < len(batch_result) < 5:
                ...
                logger.warning(f"[makeActions] Episode {batch_idx}: fewer than 5 status entries, padding with the last status")
            elif len(batch_result) == 0:
                logger.warning(f"[makeActions] Episode {batch_idx}: AirSim returned an empty list; padding with trajectory[-1]")
                logger.info(f"[makeActions] trajectory[-1] = {self.sim_states[batch_idx].trajectory[-1]['sensors']['state']['position']}")
