...
def save_to_dataset_eval(episodes, path, ori_traj_dir, llm_replies=None, instruction=None):
    ...
    if llm_replies:
        llm_dir = os.path.join(root_path, 'llm_reply')
        os.makedirs(llm_dir, exist_ok=True)
        uuid = os.path.basename(root_path.rstrip('/'))
        filepath = os.path.join(llm_dir, f'{uuid}.txt')
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(f"instruction:\n{instruction or ''}\n")
            for step_text in llm_replies:
                f.write(step_text)
...
class EvalBatchState:
    def __init__(self, batch_size, env_batchs, env, assist, model_wrapper=None):
    ...
    def update_from_env_output(self, outputs):
        ...
        for i in range(self.batch_size):
            ...
            if target_distance_increasing_for_10frames(self.distance_to_ends[i]):
                print('=' * 80)
                print(f'⚠️  Episode {i} termination reason: moving away from the target for 5 consecutive frames')
                print(f'   - current distance to target: {self.distance_to_ends[i][-1]:.2f} m')
                print(f'   - distance changes over the last 10 frames: {[f"{d:.2f}" for d in self.distance_to_ends[i][-10:]]}')
                print(f'   - current position: {observations[i][-1]["sensors"]["state"]["position"]}')
                print(f'   - target position: {self.target_positions[i]}')
                print('=' * 80)
    ...
    def check_batch_termination(self, t):
        for i in range(self.batch_size):
            if t == args.maxWaypoints:
                print('=' * 80)
                print(f'⚠️  Episode {i} termination reason: maximum step limit reached')
                print(f'   - maximum steps: {args.maxWaypoints}')
                print(f'   - current distance to target: {self.distance_to_ends[i][-1]:.2f} m')
                print(f'   - current position: {self.episodes[i][-1]["sensors"]["state"]["position"]}')
                print(f'   - target position: {self.target_positions[i]}')
                print('=' * 80)
                self.dones[i] = True
            if self.dones[i] and not self.skips[i]:
                ...
                if self.success[i]:
                    ...
                    print(f'   - final distance to target: {self.distance_to_ends[i][-1]:.2f} m (≤ 20m)')
                    print(f'   - final position: {self.episodes[i][-1]["sensors"]["state"]["position"]}')
                    print(f'   - target position: {self.target_positions[i]}')
                    print('=' * 80)
                elif self.oracle_success[i]:
                    prex = "oracle_"
                    print('=' * 80)
                    print(f'✅ Episode {i} Oracle completed successfully!')
                    print(f'   - a point along the path reached the goal vicinity')
                    print(f'   - final distance to target: {self.distance_to_ends[i][-1]:.2f} m')
                    print(f'   - final position: {self.episodes[i][-1]["sensors"]["state"]["position"]}')
                    print(f'   - target position: {self.target_positions[i]}')
                    print('=' * 80)
                else:
                    ...
                    print('=' * 80)
                    print(f'❌ Episode {i} finished unsuccessfully')
                    print(f'   - final distance to target: {self.distance_to_ends[i][-1]:.2f} m')
                    print(f'   - collision flag: {self.collisions[i]}')
                    print(f'   - final position: {self.episodes[i][-1]["sensors"]["state"]["position"]}')
                    print(f'   - target position: {self.target_positions[i]}')
                    print('   - possible reasons: collision / moving away from target / maximum steps reached')
                    print('=' * 80)
                new_traj_name = prex +  self.ori_data_dirs[i].split('/')[-1]
                new_traj_dir = os.path.join(args.eval_save_path, new_traj_name)
                llm_replies = self.model_wrapper.pop_llm_replies(i) if self.model_wrapper else None
                instruction = self.model_wrapper._ep_state[i].get('instruction', '') if self.model_wrapper else None
                save_to_dataset_eval(self.episodes[i], new_traj_dir, self.ori_data_dirs[i],
                                     llm_replies=llm_replies, instruction=instruction)
                self.skips[i] = True
                print(f'Episode {i} Saved to: {new_traj_dir}')
                print()
